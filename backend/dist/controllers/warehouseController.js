...
const transformWarehouseItem = (item) => {
    if (!item)
        return null;
    return {
...
exports.warehouseController = {
    async getAll(req, res) {
        try {
            const items = await warehouseService_1.warehouseService.getAllItems();
    ...
    async getById(req, res) {
        try {
            const item = await warehouseService_1.warehouseService.getItemById(req.params.id);
            if (!item) {
    ...
    async create(req, res) {
        try {
            ...
            const item = await warehouseService_1.warehouseService.createItem(req.body);
    ...
    async update(req, res) {
        try {
            ...
            console.log('Request Body:', JSON.stringify(req.body, null, 2));
            const item = await warehouseService_1.warehouseService.updateItem(req.params.id, req.body);
            if (!item) {
    ...
    async delete(req, res) {
        try {
            const deleted = await warehouseService_1.warehouseService.deleteItem(req.params.id);
            if (!deleted) {
                return res.status(404).json({ error: "Item not found" });
            }
    ...
    async getLowStock(req, res) {
        try {
            const items = await warehouseService_1.warehouseService.getLowStockItems();
            const transformedItems = items.map(transformWarehouseItem);
            res.json(transformedItems);
        }
        catch (error) {
